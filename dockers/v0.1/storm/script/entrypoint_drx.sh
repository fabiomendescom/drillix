...
if [ -z "$NIMBUS_ADDR" ]; then
  export NIMBUS_ADDR=$(ifconfig eth0 | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\.){3}[0-9]*).*/\2/p');
fi
...
awk -v ZOOIPS="$ZOOIPS" -v NIMBUS_ADDR="$NIMBUS_ADDR" -v UI_PORT="$UI_PORT" '{
    sub(/%zookeeper%/, ZOOIPS);
    sub(/%nimbus%/, NIMBUS_ADDR);
    sub(/%ui_port%/, UI_PORT);
    print;
}' $STORM_HOME/conf/storm.yaml.template > $STORM_HOME/conf/storm.yaml
