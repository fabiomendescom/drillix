...
sudo docker kill $1 
sudo docker rm $1 
sudo docker create --name="$1"  -e "QUEUENUMBERMESSAGES=$DRILLIX_EVENT_WORKER_QUEUENUMBERMESSAGES" -e "QUEUECONCURRENCY=$DRILLIX_EVENT_WORKER_QUEUECONCURRENCY" -e "ENVPREFIX=$DRILLIX_EVENT_WORKER_ENVPREFIX" drillix_worker_loadevents
sudo docker start $1
