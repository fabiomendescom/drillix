...
echo "---------------------------------------------"
sudo docker build -t drillix_events .   
sudo docker kill $1 
sudo docker rm $1 
sudo docker create --name="$1" -e "ENVPREFIX=$DRILLIX_EVENTS_ENVPREFIX" -e "TOPICTOPUBLISH=$DRILLIX_EVENTS_TOPICTOPUBLISH" -p 80:80 -p 443:443 drillix_events
sudo docker start $1
sudo docker logs $1 
